...
import static utilz.Constants.objectsConstants.RING;
import static utilz.Constants.objectsConstants.SPIKE;
import static utilz.Constants.objectsConstants.FLAG;
...
public class HelpMethods {
	...
	public static ArrayList<Ring> GetRings(BufferedImage img) {
		...
		for (int j = 0; j < img.getHeight(); j++)
			for (int i = 0; i < img.getWidth(); i++) {
				...
				int value = color.getGreen();
				if (value == RING)
					list.add(new Ring(i * Game.TILES_SIZE, j * Game.TILES_SIZE,RING));
			}
		return list;
	...
	public static ArrayList<Flag> GetFlags(BufferedImage img) {
		...
		for (int j = 0; j < img.getHeight(); j++)
			for (int i = 0; i < img.getWidth(); i++) {
				...
				int value = color.getBlue();
				if (value == FLAG)
					list.add(new Flag(i * Game.TILES_SIZE, j * Game.TILES_SIZE, FLAG));
			}
